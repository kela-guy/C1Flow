  ...
  return (
    <div className="space-y-2.5">
      ...
      <div className="flex items-center gap-1.5 text-[11px]">
        <span className="inline-flex items-center gap-1 px-2 py-1 rounded border border-white/10 bg-white/[0.03] text-zinc-300 shrink-0">
          <span className="inline-block size-2 rounded-full bg-zinc-500" aria-hidden />
          {t.branch.rawDetection}
        </span>
        <span className="h-px flex-1 bg-white/15" aria-hidden />
        <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded border border-white/10 bg-white/[0.03] text-zinc-200 min-w-0">
      ...
      <div className="space-y-1">
        <div className="space-y-0.5">
          <p className="text-[10px] font-medium text-zinc-500">{t.labels.affiliation}</p>
          <p className="text-[10px] text-zinc-500 leading-snug">{t.labels.affiliationHint}</p>
        </div>
            ...
            <button
              ...
              className={[
                ...
                selected
                  ...
                  : 'border-white/10 bg-white/[0.03] opacity-70 hover:opacity-100 hover:bg-white/[0.06]',
              ].join(' ')}
            >
              <span className={['inline-block size-2 rounded-full shrink-0', tw.bg].join(' ')} aria-hidden />
              <span className={['flex-1 min-w-0 truncate text-[11px]', selected ? 'text-white' : 'text-zinc-300'].join(' ')}>
                {t.affiliations[aff]}
              </span>
              <span className={['shrink-0 text-[10px] font-semibold', tw.text].join(' ')}>
      ...
      {showTail && (
        <div className="flex items-center gap-1.5 text-[11px] text-zinc-300 ps-2">
      ...
      <div className="flex items-center justify-between gap-2 pt-0.5">
        ...
        <span className="text-[11px] text-zinc-500 tabular-nums">{t.labels.estDuration(estSeconds)}</span>
      </div>
    </div>
  );
}
