...
export const spec: ComponentSpec = {
  ...
  props: [
    ...
    { name: 'subtitle', type: 'string', required: false, description: 'Secondary text below title in mono font — now only the mission/planned-action ID. The detection timestamp no longer renders here; it moved into the status slot as a color-coded ActivityTimestampChip. Truncates on overflow within the title column.' },
    { name: 'status', type: 'React.ReactNode', required: false, description: 'Status slot — a StatusChip, or the combined ActivityTimestampChip (status-colored dot + timestamp text).' },
