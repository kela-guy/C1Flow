 ...
 * REGENERATION
 * ────────────
 * Run this in a shell to regenerate the table after editing
 * palette.css (uses the OKLab → linear sRGB inverse from
 * https://bottosson.github.io/posts/oklab/, gamut-clamped to sRGB):
 *
 *   node -e "<see plan / git history for the conversion script>"
 *
 * The values committed below come from that script. APCA-tuned to
 * the same targets as the oklch() source.
 *
 * NEW CONSUMERS
 * ─────────────
